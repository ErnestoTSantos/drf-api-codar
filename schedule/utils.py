...
class Verifications:
    ...
    def is_holiday(date: date):
        logging.info(f'Fazendo requisição para BrasilAPI com a data: {date.isoformat()}')  # noqa:E501
        if settings.TESTING is True:
            logging.info('Requisição não está sendo feita pois o TESTING = True')  # noqa:E501
            if date.day == 25 and date.month == 12:
                return True
            return False

        api_request_os = os.environ.get('URL_API_HOLIDAYS', 'https://brasilapi.com.br/')  # noqa:E501
        request_api = requests.get(api_request_os + f'api/feriados/v1/{date.year}')  # noqa:E501
